...
export default function CheckOut({
  ...
  const validateStep1 = () => {
    ...
    if (!customerInfo.address || customerInfo.address.trim().length === 0) {
      ...
      newErrors.address =
        "Please provide a more detailed address (at least 10 characters)";
    }
  ...
  const handleFileUploadWithValidation = async (e) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setErrors({
        ...errors,
        qrUpload: "Please upload a valid image file (JPEG, PNG, JPG)",
      });
      setShowError(true);
  ...
  const getButtonText = () => {
    ...
    if (orderStep === 2)
      return paymentMethod === "QR" ? "Continue to Payment" : "Place Order";
  ...
  const isFormValid = () => {
    ...
    if (orderStep === 2) {
      return (
        selectedBranch &&
        paymentMethod &&
        Object.keys(validateStep2()).length === 0
      );
    }
    if (orderStep === 3) {
      ...
      <Modal.Body>
        ...
        {orderStep === 2 && (
          <>
            ...
            {errors.branch && (
              <div className="text-danger small mb-2">{errors.branch}</div>
            )}
        ...
        {orderStep === 3 && paymentMethod === "QR" && (
          ...
          >
            ...
            {errors.qrUpload && (
              <div className="text-danger small">{errors.qrUpload}</div>
            )}
            ...
            >
              ...
              <div className="text-muted small">
                Scan this QR code to make payment, then upload a proof of
                payment.
              </div>
            </div>
            <input
            ...
            {qrUploaded && (
              <Alert variant="success" className="text-center">
                <strong>Payment screenshot uploaded successfully!</strong>
                <br />
